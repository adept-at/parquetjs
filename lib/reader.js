...
class ParquetCursor {
  ...
  async next() {
    if (this.rowGroup.length === 0) {
      ...
      this.rowGroup = parquet_shredder.materializeRecords(
        this.schema,
        rowBuffer
      );
      this.rowGroupIndex++;
...
class ParquetReader {
  ...
  static async openFile(filePath, options) {
    let envelopeReader = await ParquetEnvelopeReader.openFile(
      filePath,
      options
    );
  ...
  static async openBuffer(buffer, options) {
    let envelopeReader = await ParquetEnvelopeReader.openBuffer(
      buffer,
      options
    );
    return this.openEnvelopeReader(envelopeReader, options);
  ...
  static async openS3(client, params, options) {
    let envelopeReader = await ParquetEnvelopeReader.openS3(
      client,
      params,
      options
    );
  ...
  static async openUrl(request, params, options) {
    let envelopeReader = await ParquetEnvelopeReader.openUrl(
      request,
      params,
      options
    );
...
class ParquetEnvelopeReader {
  ...
  static async openS3(client, params, options) {
    ...
    let readFn = async (offset, length, file) => {
      ...
      let res = await client
        .getObject(Object.assign({ Range }, params))
        .promise();
  ...
  readOffsetIndex(path, row_group, opts) {
    ...
    const data = this.read(
      ...
      Object.defineProperty(offset_index, "column", {
        value: column,
        enumerable: false,
      });
      if (opts && opts.cache) {
  ...
  readColumnIndex(path, row_group, opts) {
    ...
    const data = this.read(
      ...
      if (column_index.max_values) {
        column_index.max_values = column_index.max_values.map((max_value) =>
          decodeStatisticsValue(max_value, field)
        );
      }
      if (column_index.min_values) {
        column_index.min_values = column_index.min_values.map((min_value) =>
          decodeStatisticsValue(min_value, field)
        );
      }
  ...
  async readPage(column, page, records, opts) {
    ...
    if (page.offset !== undefined) {
      ...
      const offsetIndex =
        column.offsetIndex || (await this.readOffsetIndex(column, null, opts));
      column.meta_data.data_page_offset =
        offsetIndex.page_locations[page].offset;
  ...
  async readRowGroup(schema, rowGroup, columnList) {
    ...
    for (let colChunk of rowGroup.columns) {
      ...
      if (
        columnList.length > 0 &&
        parquet_util.fieldIndexOf(columnList, colKey) < 0
      ) {
        continue;
      }
  ...
  readColumnChunk(schema, colChunk, opts) {
    ...
    if (colChunk.meta_data.dictionary_page_offset) {
      ...
      dictionary = this.read(offset, size, colChunk.file_path).then(
        (buffer) =>
          decodePage({ offset: 0, buffer, size: buffer.length }, opts)
            .dictionary
      );
    }
...
function decodeStatistics(statistics, column) {
  ...
  statistics.min =
    decodeStatisticsValue(statistics.min, column) || statistics.min_value;
  statistics.max =
    decodeStatisticsValue(statistics.max, column) || statistics.max_value;
...
function decodePage(cursor, opts) {
  ...
  const headerSize = parquet_util.decodeThrift(
    pageHeader,
...
function decodeSchema(schemaElements) {
  ...
  schemaElements.forEach((schemaElement) => {
    ...
    while (
      schema.parent &&
      Object.keys(schema).length === schema.num_children
    ) {
