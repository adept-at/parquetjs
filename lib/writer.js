...
class ParquetWriter {
  ...
  async appendRow(row) {
    ...
    if (this.rowBuffer.pageRowCount >= this.envelopeWriter.pageSize) {
      encodePages(this.schema, this.rowBuffer, {
...
class ParquetEnvelopeWriter {
  ...
  constructor(schema, writeFn, closeFn, fileOffset, opts) {
    this.schema = schema;
    (this.write = writeFn), (this.close = closeFn);
    ...
    this.schema.fieldList.forEach((c, i) => {
      rowGroups.forEach((group) => {
        ...
        if (column.meta_data.columnIndex) {
          let columnBody = parquet_util.serializeThrift(
            column.meta_data.columnIndex
          );
        ...
        if (column.meta_data.offsetIndex) {
          let offsetBody = parquet_util.serializeThrift(
            column.meta_data.offsetIndex
          );
      ...
      this.writer.appendRow(row).then(
        ...
        (err) => {
          const fullErr = new Error(
            `Error transforming to parquet: ${err.toString()} row:${JSON.stringify(
              row
            )}`
          );
          fullErr.origErr = err;
...
function encodeStatistics(statistics, column) {
  ...
  statistics.min_value =
    statistics.min_value === undefined
      ? null
      : encodeStatisticsValue(statistics.min_value, column);
  statistics.max_value =
    statistics.max_value === undefined
      ? null
      : encodeStatisticsValue(statistics.max_value, column);
...
function encodePages(schema, rowBuffer, opts) {
  ...
  for (let field of schema.fieldList) {
    ...
    let first_row_index = lastPage
      ? lastPage.first_row_index + lastPage.count
      : 0;
    pages.push({
      page,
      statistics,
...
function encodeDataPage(column, values, rlevels, dlevels, statistics) {
  ...
  if (column.statistics !== false) {
    pageHeader.data_page_header.statistics = encodeStatistics(
      statistics,
      column
    );
  }

  pageHeader.data_page_header.encoding =
    parquet_thrift.Encoding[column.encoding];
...
function encodeDataPageV2(
  ...
  if (column.statistics !== false) {
    pageHeader.data_page_header_v2.statistics = encodeStatistics(
      statistics,
      column
    );
  }
  ...
  pageHeader.data_page_header_v2.encoding =
    parquet_thrift.Encoding[column.encoding];
...
function encodeColumnChunk(pages, opts) {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    if (opts.column.statistics !== false) {
      ...
      columnIndex.max_values.push(
        encodeStatisticsValue(page.statistics.max_value, opts.column)
      );
      columnIndex.min_values.push(
        encodeStatisticsValue(page.statistics.min_value, opts.column)
      );
    }
...
function encodeFooter(schema, rowCount, rowGroups, userMetadata) {
  ...
  for (let field of schema.fieldList) {
    ...
    schemaElem.repetition_type =
      parquet_thrift.FieldRepetitionType[field.repetitionType];
    ...
    if (field.originalType) {
      schemaElem.converted_type =
        parquet_thrift.ConvertedType[field.originalType];
    }
