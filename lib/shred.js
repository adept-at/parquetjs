...
exports.shredRecord = function (schema, record, buffer) {
  ...
  for (let field of schema.fieldList) {
    ...
    [...recordShredded[field.path].distinct_values].forEach((value) =>
      buffer.columnData[field.path].distinct_values.add(value)
    );
  ...
  for (let fieldName in fields) {
    ...
    for (let i = 0; i < values.length; ++i) {
      ...
      if (field.isNested) {
        ...
        data[field.path].values.push(
          parquet_types.toPrimitive(fieldType, values[i])
        );
